  ...
  methods: {
    ...
    async initWatch(criteria) {
      this.try(() => {
        ...
        // 下一行collection()里面的collection是监听的数据集合，where是监听的条件，
        this.messageListener = db.collection(collection).where(this.mergeCommonCriteria
        (criteria)).watch({
    ...
    async onConfirmSendText(e) {
      console.log('来了',this.data.groupId);
      this.try(async () => {
        if (!e.detail.value) {
          return
        }
